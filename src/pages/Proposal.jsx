...
const Proposal = React.forwardRef(({ onTransition }, ref) => {
    ...
    useEffect(() => {
        ...
        const trackVisit = async () => {
            try {
                ...
                await supabase.rpc('increment_visits', { url_token: token });

                // Append current timestamp to visit_timestamps array
                ...
                if (invite) {
                    const newTimestamps = [...(invite.visit_timestamps || []), new Date().toISOString()];
                    await supabase
                        .from('invites')
                        .update({ visit_timestamps: newTimestamps })
                        .eq('token', token);
                }

                // Set first_opened_at if this is the first visit
                await supabase
                    .from('invites')
    ...
    const handleYes = useCallback(async () => {
        ...
        // Silently save to database in background
        if (token) {
            const now = new Date().toISOString();

            // First fetch current state to handle history and first_yes_at
            supabase
                .from('invites')
                .select('yes_timestamps, first_yes_at')
                .eq('token', token)
                .single()
                .then(({ data: current, error: fetchError }) => {
                    if (fetchError) {
                    ...
                    const updates = {
                        used: true,
                        used_at: now, // Always update latest
                        yes_timestamps: [...(current?.yes_timestamps || []), now]
                    };

                    // Only set first_yes_at if it hasn't been set yet
                    if (!current?.first_yes_at) {
                        updates.first_yes_at = now;
                    }
        ...
        // Immediately proceed to celebration
        ...
        if (token) {
            try {
                ...
                if (invite) {
                    await supabase
                        .from('invites')
                        .update({ no_clicks: (invite.no_clicks || 0) + 1 })
                        .eq('token', token);
                }
            } catch (error) {
